...
# --------------Role Guessing Function--------------
def guess_role_from_resume(resume_text):
    # Add as many job roles as you want to support
    roles = [
        'data analyst', 'data engineer', 'ml engineer', 'software developer',
        'food server', 'waiter', 'waitress', 'kitchen helper', 'cashier',
        'sales associate', 'customer service', 'warehouse associate', 'cook',
        'restaurant manager', 'barista', 'receptionist', 'dishwasher'
    ]
...
# --------------Find Jobs--------------
    ...
    # --- NEW LOGIC: Auto-fill job query from resume ---
    with get_conn() as conn:
        row = conn.execute("SELECT full_text FROM uploads WHERE user_id=? ORDER BY id DESC LIMIT 1", (user["id"],)).fetchone()
    resume_text = row["full_text"] if row else ""
    suggested_role = guess_role_from_resume(resume_text) if resume_text else ""

    job_query = st.text_input("Job Title or Keywords", value=suggested_role)
    ...
    if st.button("Fetch Jobs"):
        if not job_query:
            st.warning("Could not detect a job role in your resume. Please type a keyword manually.")
        else:
